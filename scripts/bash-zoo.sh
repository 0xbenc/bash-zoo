...
uninstall_cmd() {
  ...
  if [[ $remove_all -eq 1 ]]; then
    ...
    if ! ensure_gum; then
      echo_err "gum is required for interactive selection. Install it with Homebrew:"
      echo_err "  brew install gum"
      echo_err "Or run 'bash-zoo uninstall --all' to remove everything without prompts."
      return 1
    fi
    local labels=()
